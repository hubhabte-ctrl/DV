 ...
 */
import { ColorField, FieldRow, NumberField, SelectField } from '../common/Fields';
...
function MaterialChannels({ materialId, query }: { materialId: string; query: string }) {
          ...
          c.kind === 'color' ? (
            <FieldRow key={c.key} label={c.label}>
              <ColorField
                ...
                onChange={(v) => setMaterialProp(rec.id, c.key, v, true)}
              />
            </FieldRow>
          ) : (
            ...
            <PBRSliderRow
              ...
              scale={c.scale}
              unit={c.unit}
              onChange={(v) => setMaterialProp(rec.id, c.key, v)}
              onScrub={(v) => setMaterialProp(rec.id, c.key, v, true)}
            />
          ),
        )}
      ...
      {matches('Alpha Mode') && (
        <FieldRow label="Alpha Mode">
          <SelectField
            ...
            onChange={(v) => setMaterialProp(rec.id, 'alphaMode', v)}
          />
        </FieldRow>
      )}
      ...
      {matches('rotation') && (
        <FieldRow label="UV Rotation">
          <NumberField
            ...
            step={0.05}
            unit="rad"
            onChange={(v) => setMaterialUv(rec.id, 'rotation', v)}
            onScrub={(v) => setMaterialUv(rec.id, 'rotation', v, true)}
          />
        </FieldRow>
      )}
    </>
