/**
 * Asset Studio Dock   " left sidebar.
 */
import { useState } from 'react';
import { Icons } from '../../../../app/ui/Icons';
import { SearchInput } from '../common/SearchInput';
import { setUIState, useUIState } from '@bs/engine';
import { AssetRailSettingsPopover } from './AssetRailSettingsPopover';
import { AssetSidebar } from './AssetSidebar';

const DOCK_TABS = ['folders'];
  ...
  return (
    <div className="bs-shell__leftrail bs-leftrail-root">
      <nav className="rail" role="navigation" aria-label="Studio switcher">
        <ActivityRailButtons />
        <div className="rail-bottom"><span className="rail-sep" /><AssetRailSettingsPopover /></div>
      </nav>
      <aside className="bs-dock bs-dock-inner" role="complementary" aria-label="Assets">
        <div className="bs-dock__tabs" role="tablist">
          {DOCK_TABS.map((t) => (
            <button key={t} type="button" role="tab" id={`bs-dock-tab-${t}`} aria-selected={tab === t}
        ...
        <div className="bs-dock__header"><span className="bs-dock__title">Folders</span></div>
        <SearchInput value={search} onChange={setSearch} placeholder="Search folders…" />
        <div className="bs-dock__scroll" id="bs-dock-panel" role="tabpanel">
          <AssetSidebar search={search} />
        </div>
      </aside>
    </div>
  );
